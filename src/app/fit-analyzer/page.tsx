...
import { FitAnalyzerClient } from "./FitAnalyzerClient";

export default async function FitAnalyzerPage() {
  ...
  if (!userId) {
    return (
      <section className="panel rounded-3xl p-8">
        ...
        <h2 className="mt-3 text-3xl font-semibold text-white">Fit analyzer</h2>
        <p className="mt-4 max-w-2xl text-slate-300">
          Sign in with EVE and sync at least one character to analyze a fit for one pilot in the browser.
        </p>
        <Link
  ...
  if (characters.length === 0) {
    return (
      <section className="panel rounded-3xl p-8">
        ...
        <h2 className="mt-3 text-3xl font-semibold text-white">Fit analyzer</h2>
        <p className="mt-4 max-w-2xl text-slate-300">
          Sync at least one character first. The fit analyzer depends on stored skills and attributes before it can call the analysis API.
        </p>
        <Link
          className="mt-6 inline-flex rounded-full bg-flux px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-emerald-300"
          href="/dashboard"
